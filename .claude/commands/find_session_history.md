...
SESSION_DIR=$(python - <<'PY'
import json
import os
from pathlib import Path

config = json.loads(Path("config.json").read_text())
session_dir = config["find_session_history"]["session_dir"]
print(os.path.expanduser(os.path.expandvars(session_dir)))
PY
)
...
**Important:**
- Read the history path from `config.json`; do not hardcode it in this command file
...
### Step 3: Copy the session file to prompts/ (local-only)
...
Files written under `prompts/` here are local-only manager artifacts. Never stage, commit, or push the copied `.jsonl` or generated transcript.
...
- Read the session-history directory from `config.json`, not from a hardcoded path in this command
...
- Anything under this repo's `prompts/` folder is local-only scratch/report material and must never be committed
