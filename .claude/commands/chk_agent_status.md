...
Use this skill when the owner asks for:
...
The target session is always the **active worker** resolved from `config.json`. Do not ask for a session name.
...
- Respect the resolved worker's original `session`, `send_method`, and `working_dir`. If that worker is unhealthy, report or recover it instead of switching workers.
...
If the state or the last delegated task is still unclear, extend to:

```bash
tmux capture-pane -t <session> -p -S -100
```

Only for real diagnosis, extend further:
...
Do not jump to a deep capture unless the quick and medium slices still fail to answer the question.
