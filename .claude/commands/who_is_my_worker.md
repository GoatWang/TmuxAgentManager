---
description: Identify the active worker for the current bot profile, including env file, FIRST_PROMPT match, tmux session, and working_dir.
argument-hint: ""
...
- which tmux session / working_dir / send method belongs to the current worker
...
`$ARGUMENTS` — none expected. Ignore extra arguments unless the owner explicitly asks for a deeper explanation.

## Goal

Tell the owner exactly which worker is active **right now**, **why** that worker was selected, and whether the worker's tmux session is healthy.
...
1. **Exact worker-name match in `FIRST_PROMPT`**
   - If the prompt mentions a worker name from `config.json`, use that worker.
