...
- Do not switch away from the active worker unless the owner explicitly asked to scan a different worker
...
Get the resolved worker's `session`, `send_method`, and `working_dir`.

### Step 2: Detect worker agent type

Use metadata plus a small capture to identify what agent is running:
...
```bash
sleep 15
tmux capture-pane -t <session> -p -S -250
```

If the needed output is incomplete, extend once to:
```bash
...
- Only the owner may explicitly approve deviating from this workflow.
...
- Follow the development workflow above as mandatory manager process; do not ask the worker to skip required steps unless the owner explicitly approves that deviation
- Respect the worker's original settings from `config.json` / `FIRST_PROMPT`
- Discuss project questions with the worker before asking the owner
```
...
- **Always read `config.json`** first — never hardcode session names or paths.
- Treat the generated cheat sheet as the manager-facing workflow contract for that worker, not just a reference note.
- Preserve the current worker's original settings unless the owner explicitly asks for worker-config meta-work.
- If a follow-up question remains after the scan, ask the worker first before asking the owner.
