...
Spawn a new tmux session running the worker in the configured `working_dir`, using `resume` to carry over context from a previous session.
...
- **working_dir** — the directory where the worker should run
...
Create a new detached tmux session in the worker's configured `working_dir`:

```bash
tmux new-session -d -s <new_session_name> -c <working_dir>
```
...
- **Always read `config.json`** for the worker's `working_dir` — never hardcode paths
