...
Use this command when the owner wants a new worker added to this repo's manager configuration.
...
- `/add_worker OysterunQA <telegram_allowed_user_id> <telegram_bot_token>`
- `/add_worker OysterunQA <telegram_allowed_user_id> <telegram_bot_token> Oysterun`
...
4. appending the new worker to `config.json`
...
- `config.json` — canonical worker registry
...
- workers live in `config.json` under `workers`
...
- existing workers and existing env profiles must keep their original settings unless the owner explicitly asks to change them
...
- `worker_name` — also used as the tmux `session` name unless the owner explicitly asks for something different
...
Read `config.json` and choose the source worker to clone:
...
- `working_dir`: copied from template worker
- `session`: `<worker_name>` unless the owner explicitly gave a different session name
...
- do **not** alter the existing workers' `name`, `session`, `send_method`, `project`, or `working_dir`
- do **not** overwrite the legacy top-level `worker` object unless the owner explicitly wants the new worker to become the default `workers[0]`
...
ALLOWED_PATHS=/
FIRST_PROMPT=Default worker: use the worker named <worker_name> from config.json for delegation, monitoring, and status checks. Respect the original worker settings and do not switch workers unless the owner explicitly asks for agent-manager meta-work.
```

Do not modify existing env files unless the owner explicitly asks to repoint an existing bot profile.

### Step 7: Final verification

After writing the files:

1. read back the new worker entry from `config.json`
...
- Copy only the template fields that should stay aligned across workers: `project`, `working_dir`, and `send_method`
...
- This command creates config for a new worker profile; it does not automatically restart the Telegram bot or spawn a tmux worker session unless the owner asks for that separately
