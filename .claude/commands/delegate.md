...
Use this command when the owner gives a non-trivial project task. This command exists to prevent you from slipping into direct implementation.
...
```bash
cd <working_dir> && pwd && git rev-parse --show-toplevel && git status --short
```
...
If the receipt is unclear, escalate to a medium capture such as `-80` to `-100`. Do not jump straight to a deep forensic capture.
...
- asking the worker follow-up questions before asking the owner for more input
- reviewing diffs
...
- **Always read `config.json`** to get the worker config — never hardcode session names
- Respect the original worker settings from `config.json` / `FIRST_PROMPT`; if that worker has a problem, recover or report it instead of switching workers
