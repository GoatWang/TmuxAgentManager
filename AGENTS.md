...
**You must NEVER read, write, edit, or delete ANY file inside the worker's `working_dir`.** This includes — but is not limited to:
...
- **Any other file or directory** under the worker's `working_dir` path (as defined in `config.json`)
...
- `CLAUDE.md`, `config.json`, `.claude/commands/`, `.claude/skills/`, and other TmuxAgentManager meta-files

**If you catch yourself about to read or edit a file under a worker's `working_dir` — STOP. Delegate instead.**
...
- **File I/O** — Read, Write, Edit for agent manager config files only (CLAUDE.md, config.json, commands)
- **Web** — WebSearch, WebFetch for research
- **Skills** — Project-local slash commands

### Safety Rules
- **NEVER** read, write, edit, or delete ANY file inside the worker's `working_dir` — this includes the worker's `CLAUDE.md`, `AGENTS.md`, `.claude/` folder, and all project files. Delegate changes to the worker instead.
